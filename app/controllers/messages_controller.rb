...
class MessagesController < ApplicationController
  ...
  def index
    ...
    return head status: :forbidden unless params[:user_id] == session[:user_id].to_s || User.find_by(id: session[:user_id]).is_admin
  ...
  def create
    return head status: :forbidden if session[:user_id].nil? || !User.find_by(id: session[:user_id]).is_admin
